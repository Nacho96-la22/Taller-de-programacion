...
//-------------------------------------------- Inciso C -------------------------------------------
procedure incisoC (vp:vectorPuntaje; dimL:integer);
    ...
    begin
        ...
        for i:= 1 to dimL do begin
            if (vp[i].MaxPuntaje <> -1) then begin
    ...
    procedure ordenadaPuntaje (var vp:vectorPuntaje; dimL:Integer);
    var
        i,j, pos:integer;
...
begin
    ordenadaPuntaje (vp,dimL);
    imprimirOrdenada (vp,dimL);
end;}
//-------------------------------------------- Inciso B -------------------------------------------
procedure incisoB (var vp:vectorPuntaje; v:vectorGenero; var dimL:integer);

    procedure imprimirPuntaje (vp:vectorPuntaje; dimL:integer);
    var
        i:Integer;
    begin
        writeln ('-----------------------------------------------------------------------------');
    ...
    procedure recorrerlista (var max:regMayorPuntaje; l:lista);
    begin
        max.MaxPuntaje:= -1;
        while (l <> nil) do begin
            if (max.MaxPuntaje < l^.datos.puntajePromedio) then begin
                max.genero:= l^.datos.codGenero;
                max.codPeliMayor:= l^.datos.codPelicula;
                max.MaxPuntaje:= l^.datos.puntajePromedio;
            end;
            l:= l^.sig;
        end;
    end;
    procedure PeliculaMayorPuntaje (var vp:vectorPuntaje; v:vectorGenero; var dimL:integer);
    var
        i:integer;
    begin

        for i:= 1 to 8 do  
            recorrerlista(vp[i], v[i]);
    
    end;

begin
    PeliculaMayorPuntaje (vp, v, dimL);
    imprimirPuntaje (vp,dimL);
end;
//-------------------------------------------- Inciso A -------------------------------------------
